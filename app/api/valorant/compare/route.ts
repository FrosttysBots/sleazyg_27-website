...
export async function POST(request: Request) {
    try {
        ...
        const riotName = riotId.substring(0, hashIndex).trim();
        const riotTag = riotId.substring(hashIndex + 1).trim();

        if (!riotName || !riotTag) {
            return NextResponse.json(
                { error: "Invalid Riot ID format. Both name and tag are required." },
                { status: 400 }
            );
        }
        ...
        const [accountRes, mmrRes] = await Promise.all([
            fetch(`${API_BASE}/valorant/v1/account/${encodeURIComponent(riotName)}/${encodeURIComponent(riotTag)}`, {
                headers,
                cache: "no-store",
            }),
            fetch(`${API_BASE}/valorant/v2/mmr/${REGION}/${encodeURIComponent(riotName)}/${encodeURIComponent(riotTag)}`, {
                headers,
                cache: "no-store",
            }),
        ]);
        ...
        if (accountJson.status !== 200) {
            if (accountJson.status === 404) {
                return NextResponse.json(
                    { error: "Player not found. Check the Riot ID and try again." },
                    { status: 404 }
                );
            }
            return NextResponse.json(
                { error: "Failed to fetch player data", details: accountJson },
                { status: 500 }
            );
        }
        ...
        const matchesRes = await fetch(
            `${API_BASE}/valorant/v3/matches/${REGION}/${encodeURIComponent(riotName)}/${encodeURIComponent(riotTag)}?mode=competitive&size=20`,
            {
                headers,
                cache: "no-store",
            }
        ...
        for (const match of matches) {
            ...
            const player = match.players.all_players.find(
                (p) => p.puuid === puuid || (p.name.toLowerCase() === riotName.toLowerCase() && p.tag.toLowerCase() === riotTag.toLowerCase())
            );
